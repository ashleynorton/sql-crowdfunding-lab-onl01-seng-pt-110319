...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, projects.funding_goal, SUM(pledges.amount) FROM users INNER JOIN pledges ON projects.id = pledges.project_id HAVING SUM(pledges.amount) > projects.funding_goal ORDER BY projects.title;"
end
